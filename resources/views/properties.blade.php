...
<body class="bg-gray-50 font-sans">
            ...
            <div class="p-8 pb-4">
                <div class="flex items-center justify-between">
                    <div class="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Search properties..." id="searchInput" oninput="filterProperties()" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div class="flex items-center space-x-2 ml-4">
                        <i class="fas fa-filter text-gray-400"></i>
                        <select id="statusFilter" onchange="filterProperties()" class="border border-gray-300 rounded-lg px-3 py-2 text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
    ...
    <script>
        ...
        function openEditModal(propertyId) {
            ...
            document.getElementById('editModalTitle').textContent = `Edit ${property.name}`;
            document.getElementById('editPropertyName').value = property.name;
            document.getElementById('editPropertyType').value = property.type.toLowerCase().replace(' ', '-');
            
            // Parse address
            const addressParts = property.address.split(', ');
            document.getElementById('editStreetAddress').value = addressParts[0] || '';
            document.getElementById('editCity').value = addressParts[1] || '';
            document.getElementById('editState').value = addressParts[2]?.split(' ')[0] || '';
            document.getElementById('editZipCode').value = addressParts[2]?.split(' ')[1] || '';
            
            document.getElementById('editTotalUnits').value = property.totalUnits;
            document.getElementById('editAverageRent').value = property.averageRent.replace('$', '').replace(',', '');
        ...
        editPropertyForm.addEventListener('submit', (e) => {
            ...
            if (!currentEditingProperty) return;
            ...
            const formData = {
                ...
                totalUnits: parseInt(document.getElementById('editTotalUnits').value),
                averageRent: parseFloat(document.getElementById('editAverageRent').value),
                status: document.getElementById('editPropertyStatus').value,
                description: document.getElementById('editDescription').value
            };

            // Update property data
            const property = propertyData[currentEditingProperty];
            property.name = formData.name;
            property.type = formData.type.charAt(0).toUpperCase() + formData.type.slice(1).replace('-', ' ');
            property.address = `${formData.streetAddress}, ${formData.city}, ${formData.state} ${formData.zipCode}`;
            property.totalUnits = formData.totalUnits;
            property.averageRent = `$${formData.averageRent.toLocaleString()}`;
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                if (!viewPropertyModal.classList.contains('hidden')) {
                    closeViewModalFunction();
                }
                if (!editPropertyModal.classList.contains('hidden')) {
                    closeEditModalFunction();
                }
                if (!addPropertyModal.classList.contains('hidden')) {
                    closeModalFunction();
                }
            }
        });
